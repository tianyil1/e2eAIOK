...
from module.nlp.bert_embedding_super import SuperBertEmbeddings,SuperRoBERTaEmbeddings
...
class BertPreTrainedModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *inputs, **kwargs):
        ...
        for key in state_dict.keys():
            ...
            if 'lm_head.dense' in key:
                new_key = key.replace('lm_head.dense','bert.dense_fit')
...
class SuperBertModel(BertPreTrainedModel):
    def __init__(self, model_name,config, fit_size=768):
        super(SuperBertModel, self).__init__(config)
        if model_name =='bert':
            self.embeddings = SuperBertEmbeddings(config)
        elif model_name == 'roberta':
            self.embeddings = SuperRoBERTaEmbeddings(config)
...
class SuperBertForQuestionAnswering(BertPreTrainedModel):
    def __init__(self, model_name,config):
        ...
        self.bert = SuperBertModel(model_name,config)
